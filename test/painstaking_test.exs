defmodule PainStakingTest do
  ...
  test "simple arb" do
    ...
    assert PainStaking.arb([{"lions", [prob: 0.85], [us: "-110"]}]) ==  no_arb_error, "No arbitrage exists on a single, even positive EV  outcome"
    assert PainStaking.arb([{"lions", [prob: 0.85], [us: "-110"]}, {"bears", [prob: 0.15], [us: "-110"]}]) ==  no_arb_error, "Standard US odds exhibit no arbitrage"
    assert PainStaking.arb([{"lions", [prob: 0.85], [us: "-107"]}, {"bears", [prob: 0.15], [us: "+110"]}]) ==  {:ok, [{"lions", 51.69}, {"bears", 47.62}], 0.69}, "Can make a small profit on small arbitrage"
    assert PainStaking.arb([{"nadal", [prob: 0.50], [us: "-161"]}, {"murray", [prob: 0.25], [us: "+350"]}, {"becker", [prob: 0.01],  [us: "+632"]}]) ==  {:ok, [{"nadal", 61.69}, {"murray", 22.22}, {"becker", 13.66}], 2.43}, "Multi-ways are harder to spot so may exhibit more profit"
  end
  ...
  test "independent simultaneous sim_win" do
    ...
    {:ok, win} = PainStaking.sim_win([small_edge])
    assert win <= 1.00, "A small edge on a small bankroll cannot make a ton of money"
    {:ok, win} = PainStaking.sim_win([unlikely, small_edge])
    assert win <= 50.00, "Bigger variance when you include an unlikely result"
    assert PainStaking.sim_win([always_win], 1) == {:ok, 90.91}, "If the result is known, you get full value."
    assert PainStaking.sim_win([always_win]) == {:ok, 90.91}, "Even when you repeat it many times"
    assert PainStaking.sim_win([always_win, always_lose], 100) == {:ok, 90.91}, "Same when you add one which cannot win"
  end
  ...
  test "many horses sim_win" do
    ...
    {:ok, win} = PainStaking.sim_win([chalk,dark,glue], 100, [independent: false])
    assert win <= 10.00, "Get a simulated win with only the positive expectation results"
    {:ok, win} = PainStaking.sim_win([chalk,stalk,dark,glue], 100, [independent: false])
    ...
    {:ok, win} = PainStaking.sim_win(biased_coin, 1000, [bankroll: 100_000, independent: false])
