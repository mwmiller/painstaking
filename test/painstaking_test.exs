defmodule PainStakingTest do
  ...
  test "single kelly" do
    ...
    assert kelly([no_edge]) == {:error, "No suitable positive expectation edges found."},
           "No recommended bet for non-advantage situations"
    ...
    assert kelly([no_payout]) == {:error, "No suitable positive expectation edges found."},
           "Never bet if we won't get paid."
  end
  ...
  test "many horses kelly" do
    ...
    assert kelly([chalk, stalk, dark, glue]) ==
             {:ok, [{"dark", 4.00}, {"chalk", 75.00}, {"glue", 1.00}, {"stalk", 20.00}]},
           "stalking horse is included"

    assert kelly([stalk]) == {:error, "No suitable positive expectation edges found."},
           "Even though it is negative EV when considered alone"

    assert kelly([chalk, dark, glue]) == {:ok, [{"dark", 2.06}, {"chalk", 37.44}, {"glue", 0.41}]},
           "Leaving it off wildly changes the results"

    assert kelly([chalk, stalk, dark]) ==
             {:ok, [{"dark", 3.73}, {"chalk", 69.81}, {"stalk", 18.16}]},
           "Where dropping a positive EV unlikely winner is less dramatic"
  end

  test "simultaneous independent kelly" do
    # Events from the Whitrow, 2007 paper plus 13 as a negative EV example
    events = [
      {"1", [prob: 0.470], [eu: 2.50]},
  ...
  test "simple arb" do
    ...
    assert arb([{"lions", [prob: 0.85], [us: "-110"]}]) == no_arb_error,
           "No arbitrage exists on a single, even positive EV  outcome"

    assert arb([{"lions", [prob: 0.85], [us: "-110"]}, {"bears", [prob: 0.15], [us: "-110"]}]) ==
             no_arb_error,
           "Standard US odds exhibit no arbitrage"

    assert arb([{"lions", [prob: 0.85], [us: "-107"]}, {"bears", [prob: 0.15], [us: "+110"]}]) ==
             {:ok, [{"lions", 52.05}, {"bears", 47.95}], 0.69},
           "Can make a small profit on small arbitrage"

    assert arb([
             {"nadal", [prob: 0.50], [us: "-161"]},
             {"murray", [prob: 0.25], [us: "+350"]},
             {"becker", [prob: 0.01], [us: "+632"]}
           ]) == {:ok, [{"nadal", 63.22}, {"murray", 22.78}, {"becker", 14.00}], 2.49},
           "Multi-ways are harder to spot so may exhibit more profit"
  end
  ...
  test "simple ev" do
    ...
    assert ev([neg_ev, no_ev, pos_ev]) ==
             {:ok, [{"neg edge", 98.0}, {"no edge", 100.0}, {"pos edge", 102.0}]},
           "Difference from 100 is the expected win"
  end
end
